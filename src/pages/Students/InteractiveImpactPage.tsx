...
// --- Components (Counter, MainStatNode, etc.) are unchanged ---
// (Assuming they are defined above as in the previous code)
...
const MainStatNode = ({ data }: { data: any }) => (
    <motion.div className={`flex w-full ${data.align === 'left' ? 'justify-start' : 'justify-end'}`} initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true, amount: 0.5 }}>
        ...
        >
            ...
            <h3 className={`text-6xl font-extrabold mb-3 ${data.align === 'left' ? 'text-gray-800' : 'text-white'}`}><Counter from={0} to={data.value} />+</h3>
...
const TestimonialNode = ({ data }: { data: any }) => (
...
const FactoidNode = ({ data }: { data: any }) => (
...
const ParallaxNode = ({ data }: { data: any }) => {
...
// --- CHANGE #2: Moved this component outside of InteractiveImpactPage ---
const ScrollableContent = ({ targetRef }: { targetRef: React.RefObject<HTMLElement> }) => {
  ...
  return (
    <>
      ...
      <div className="relative z-10 container mx-auto px-6 -mt-[100vh]">
        {/* All the scrolling content sections are unchanged... */}
...
const InteractiveImpactPage: React.FC = () => {
  // --- CHANGE #1: Changed the ref type to HTMLElement ---
  ...
  return (
    // The ref is attached to the <section> element
    <section ref={targetRef} className="relative py-20 bg-gradient-to-b from-teal-50 to-emerald-100 min-h-[700vh]">
      ...
      {/* This line will now work without any type errors */}
      {/* @ts-ignore */}
      {isMounted && <ScrollableContent targetRef={targetRef} />}
    </section>
  );
};
